...
source(here("my_plot_lime_features.R")) #albert original: source(here("./code/my_plot_lime_features.R"))
```
...
tic("logit")
for(i in 1:5){
  train = dt %>% slice(-test_index[[i]]) %>% as.data.frame()
  test = dt %>% slice(test_index[[i]]) %>% as.data.frame()
...
pred_dt = bind_rows(pred_dt_ls)
saveRDS(pred_dt, here("../pep_risk_app", "data", "pred_dt_logit.rds"))
```
...
# Calibration plot
# pred_dt %>%
#   ggplot(aes(x = pred, y = as.numeric(y) - 1)) +
#   geom_point() +
...
pred_dt = bind_rows(pred_dt_ls)
saveRDS(pred_dt, here("../pep_risk_app", "data", "pred_dt_gbm.rds"))
```
...
pred_dt = bind_rows(pred_dt_ls)
saveRDS(pred_dt, here("../pep_risk_app", "data", "pred_dt_rf.rds"))
```
...
#             metric = "Kappa") # metric for selecting tuning parameters
# varImp(fit)
# saveRDS(fit, here("pep_risk_app", "data", "logit_model.rds"))
...
varImp(fit)
saveRDS(fit, here("../pep_risk_app", "data", "gbm_model.rds")) #EMILY uncommented
# fit = train(pep ~ ., data = train_impute %>% select(-patient_id),
#             method = "rf",
#             trControl = fitControl,
#             verbose = FALSE,
...
# saveRDS(train, here("pep_risk_app", "data", "train_new.rds"))
...
```{r eval = F}
# Train rf models on subsets of therapies
fit_sub = list()
patients_sub = trt_randomized_groups %>% filter(trt_ah) %>% pull(patient_id)
fit_sub[["Aggressive hydration only"]] = train(pep ~ ., data = train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id), 
                                               method = "rf", 
                                               trControl = fitControl,
                                               verbose = FALSE,
                                               ntree = 500)
patients_sub = trt_randomized_groups %>% filter(trt_indo) %>% pull(patient_id)
fit_sub[["Indomethacin only"]] = train(pep ~ ., data = train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id), 
                                       method = "rf", 
                                       trControl = fitControl,
                                       verbose = FALSE,
                                       ntree = 500)
...
fit_sub[["Aggressive hydration and indomethacin"]] = train(pep ~ ., data = train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id), 
                                                           method = "rf", 
                                                           trControl = fitControl,
                                                           verbose = FALSE,
                                                           ntree = 500)
...
fit_sub[["PD stent only"]] = train(pep ~ ., data = train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id), 
                                   method = "rf", 
                                   trControl = fitControl,
                                   verbose = FALSE,
                                   ntree = 500)
...
fit_sub[["Indomethacin and PD stent"]] = train(pep ~ ., data = train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id), 
                                               method = "rf", 
                                               trControl = fitControl,
                                               verbose = FALSE,
                                               ntree = 500)
saveRDS(fit_sub, here("../pep_risk_app", "data", "rf_model_trt.rds"))
```

```{r eval = F}
# Train logistic regression to get estimate of trt effect
fit_lr_sub = list()
patients_sub = trt_randomized_groups %>% filter(trt_ah) %>% pull(patient_id)
fit_lr_sub[["Aggressive hydration only"]] = glm(pep ~., family = binomial(link='logit'), data= train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id))

patients_sub = trt_randomized_groups %>% filter(trt_indo) %>% pull(patient_id)
fit_lr_sub[["Indomethacin only"]] = glm(pep ~., family = binomial(link='logit'), data= train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id))

patients_sub = trt_randomized_groups %>% filter(trt_ah_indo) %>% pull(patient_id)
fit_lr_sub[["Aggressive hydration and indomethacin"]] = glm(pep ~., family = binomial(link='logit'), data= train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id))

patients_sub = trt_randomized_groups %>% filter(trt_pd) %>% pull(patient_id)
fit_lr_sub[["PD stent only"]] = glm(pep ~., family = binomial(link='logit'), data= train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id))

patients_sub = trt_randomized_groups %>% filter(trt_indo_pd) %>% pull(patient_id)
fit_lr_sub[["Indomethacin and PD stent"]] = glm(pep ~., family = binomial(link='logit'), data= train_impute %>% filter(patient_id %in% patients_sub) %>% select(-patient_id))

trt_names = c("aggressive_hydration", "indomethacin_nsaid_prophylaxis", "indomethacin_nsaid_prophylaxis", "pancreatic_duct_stent_placement")
for(i in seq_along(fit_lr_sub)){
  trt_name = trt_names[i]
  model = fit_lr_sub[[i]]
  
  # CI for odds ratio
  or = exp(coef(model)[trt_name])
  ci = suppressWarnings(exp(confint(model)))
  print(paste(or, "95% CI:", ci[trt_name, 1], ",", ci[trt_name, 2]))
}
```
